...
# apps
rtkrcv: $(LIB) $(addprefix $(OUTPUT_DIR)/app/, $(SRC_NAMES_RTKRCV:%.c=%.o))
	$(CC) $(abspath $^) -o $(OUTPUT_DIR)/$@ $(LDLIBS)  -L$(OUTPUT_DIR) -lrtk

rnx2rtkp: $(LIB) $(addprefix $(OUTPUT_DIR)/app/, $(SRC_NAMES_RNX2RTKP:%.c=%.o))
	$(CC) $(abspath $^) -o $(OUTPUT_DIR)/$@ $(LDLIBS) -L$(OUTPUT_DIR) -lrtk

pos2kml: $(LIB) $(addprefix $(OUTPUT_DIR)/app/, $(SRC_NAMES_POS2KML:%.c=%.o))
	$(CC) $(abspath $^) -o $(OUTPUT_DIR)/$@ $(LDLIBS) -L$(OUTPUT_DIR) -lrtk

convbin: $(LIB) $(addprefix $(OUTPUT_DIR)/app/, $(SRC_NAMES_CONVBIN:%.c=%.o))
	$(CC) $(abspath $^) -o $(OUTPUT_DIR)/$@ $(LDLIBS) -L$(OUTPUT_DIR) -lrtk

str2str: $(LIB) $(addprefix $(OUTPUT_DIR)/app/, $(SRC_NAMES_STR2STR:%.c=%.o))
	$(CC) $(abspath $^) -o $(OUTPUT_DIR)/$@ $(LDLIBS) -L$(OUTPUT_DIR) -lrtk
